...
const defaultPoolTemplate = {
  ...
  // Shape: { name, rewardTokenCoingeckoId, amountDataKey } for gaugeVersion === 1
  // Shape: { name, rewardTokenCoingeckoId, amountDataKey, rewardTokenAddress, rewardTokenDecimals } for gaugeVersion === 2
  additionalRewards: [],
  // Same as additionalRewards, only those rewards aren't actively distributed anymore.
  // Those old rewards aren't displayed in the pool list anymore, but this info is used
  // to retrieve claimable amounts for users who partook in these.
  // Shape: { name, rewardTokenCoingeckoId } for gaugeVersion === 1
  // Shape: { name, rewardTokenCoingeckoId, rewardTokenAddress, rewardTokenDecimals } for gaugeVersion === 2
